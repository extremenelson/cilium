...
func (c *Client) EndpointLabelsPatch(id string, toAdd, toDelete models.Labels) error {
	...
	disabledLbl := labels.NewLabelsFromModel(currentCfg.Status.Realized.Disabled)
	derivedLbl := labels.NewLabelsFromModel(currentCfg.Status.Derived)
	securityRelevantLbl := labels.NewLabelsFromModel(currentCfg.Status.SecurityRelevant)
	for _, lbl := range toAdd {
		lblParsed := labels.ParseLabel(lbl)
		if _, found := disabledLbl[lblParsed.Key]; found {
	...
	for _, lbl := range toDelete {
		lblParsed := labels.ParseLabel(lbl)
		if _, found := derivedLbl[lblParsed.Key]; found {
			disabledLbl[lblParsed.Key] = lblParsed
			continue
		}
		if _, found := securityRelevantLbl[lblParsed.Key]; found {
			disabledLbl[lblParsed.Key] = lblParsed
			continue
		}
		if _, found := userLbl[lblParsed.Key]; found {
	...
	currentCfg.Spec.Disabled = disabledLbl.GetModel()
